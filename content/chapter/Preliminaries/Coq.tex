...
These definitions represent two different kinds of functions: \cinl{isZero} is a non-recursive function, whereas \cinl{add} is a representative for recursive functions.
...
We use the predefined definition for lists; as a code convention we name type variables using upper-case letters.
...
Constructors of polymorphic functions need to be applied to more arguments than we are used to in functional languages like Haskell.
...
We use the tactic \cinl{discriminate} on hypotheses that state two structurally different expressions to be equal, like \cinl{z = s x}.
