...
Dissecting this type signatures gives us the following parts: given a proposition \cinl{P} on lists, we need to supply two proofs.
A proof of type \cinl{P nil} in case of an empty list and a proof of type \cinl{P (cons x xs} otherwise, where \cinl{x} and \cinl{xs} are universally quantified.
The first proof is the base case of the induction principles for lists, while the second proof is the inductive case.
That is, in the second proof we assume that the proposition already holds for an arbitrary but fixed list \cinl{xs} and can then prove that the proposition also holds for \cinl{cons x xs}.
...
When using backward-reasoning, we have to use the function \cinl{list_ind} on a function of type \cinl{l -> P  l}, which is the type of our lemma, when we instantiate that the argument list \cinl{l} with the concrete list \cinl{xs}.
...
Given a list \cinl{ys} and the induction hypothesis that the proposition already holds for that \cinl{ys}, we need to prove that the proposition holds for \cinl{cons y ys} as well.
...
That is, we need to introduce these variables ourselves using \cinl{intros} to specify names for the quantified variables first, but can then prove the goal using the induction hypothesis like in the first variant.
...
The position type \cinl{Pos} is a dependent type: the type depends on a value of type \cinl{Shape} as first argument
...
The definition of \cinl{from_One} uses an empty pattern match, because there are no possible values of type \cinl{One!$_{\text{P}}$! tt} that is just a type synonym for \cinl{Empty}.
...
Note that in contrast to the definition of \cinl{map_length}, we introduced all variables on the right-hand side of the type signature.
After introduced these quantified variables as hypotheses, we can directly simplify the function applications of \cinl{from_One} and \cinl{to_One} as they disregard the given argument.
