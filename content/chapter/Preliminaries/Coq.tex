...
As some of these solutions will lead us to dependently typed programming, we will take a look at how to formalise properties about programs and how to prove them.
...
Note that the definition of \cinl{isZero} additionally uses the predefined \cinl{bool} type as result, and that we can group successive arguments of the same type in the type signature.
...
An example for a recursive function that Coq's termination checker does accept, is the following artificial definition that decreases on one of its arguments at each recursive call, but does not have one designated argument that decreases for each call.
...
Moving on with our definitions on Peano numbers above, we can introduce a \cinl{Notation} that allows us to write more natural expressions like \cinl{p1 + p2} for
...
Constructors of polymorphic functions need to be applied to more arguments than we are used to from functional languages such as Haskell.
...
The definition above works when we apply a type like \cinl{peano} explicitly or instruct Coq to infer the argument by using an underscore (\cinl{_}).
...
The introduction pattern \cinl{[ | y ys H ]} describes the naming conventions for two subgoals: we do not supply any names for the first goal and the three names \cinl{y}, \cinl{ys} and \cinl{H} for the \cinl{cons}-subgoal.
...
As stated in the beginning of the last section, the property about map preserving the length of the list is not surprising.
In the next two sections we take a look at how to encode the length of the list in its type and discuss that due to this encoding, the property of map not manipulating the length of the list becomes clear from the type signature alone.
...
When we defined the function \cinl{vmap}, we observed that we cannot use a more general type using two type variables \cinl{n m : peano} for the input and output list.
...
Since we will use the same types \cinl{A B}, Peano numbers \cinl{n m} and a function \cinl{f : A -> B} for several proofs, we use a section to declare these variables upfront so we do not need to introduce them for each proof or definition again.
