...
\subsubsection{A First Attempt to Model Call-Time Choice with Free Monads}
...
do cannot implement the approach by \citeauthor{pirog18syntax} in Coq.
...
\citet{antoy2017proving} present ideas to model Curry programs in
Agda.
They use to explicit representations of non-determinism, which they call \emph{set of
  values} and \emph{planned choices}, respectively.
The first approach translates a non-deterministic Curry function into
...
Their model is a specialisation of the generic model that we use:
instantiating \cinl{Free Sh Ps A} with a variant of the
non-determinism effect \cinl{ND} without \cinl{failed} results in a
representation isomorphic to their data type \emph{ND}.
Instead of using a monadic interface they define operators \emph{$*}
and \emph{*$*}, where the former corresponds to \emph{Functor} method
\emph{fmap} and the latter to the monadic operation \emphl{>>=}.
The translation for the second approach adds an extra argument to the
translated function.
...
\cite{antoy2017proving}
\cite{bunkenburg2019modeling}
\cite{cleva2004logic} proof calculus for CRWL
