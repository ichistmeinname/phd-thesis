...
We proceed by stripping away the \cinl{Cons}-constructor and take another look at \cinl{fxs} by destructing the induction hypothesis \cinl{IH}, which produces two cases: one for \cinl{pure} and one for \cinl{impure}.
...
The assumption \cinl{s : Zero!$_\text{S}$!} is once again a bogus assumption, so we clear the last remaining goal by using \cinl{contradiction} again.
...
We can, however, prove the \cinl{impure}-cases using \cinl{contradiction} again, but we need to have a position as an
assumption.
...
The part we like to investigate further is the custom proof principle they use to prove properties about programs modelled as f-and-m-algebras.
