...
If the input list \cinl{defined}, we can
...
\cinl{undefined} in case of an empty list.
...
\paragraph{Strictly Positive Types}
...
a strict positive occurrence of a type as follows: a type $\tau$
...
Again, think about the definition of \cinl{NonStrictlyPos}, we now
know that \cinl{NonStrictlyPos} occurs ``left of an arrow'' in the
function type \cinl{NonStrictlyPos -> nat} that is the first argument
of the \cinl{con}-constructor.
...
open: (1) Why is our definition \cinl{List} non-strictly positive as
well, there is not even an arrow in the arguments' type of
\cinl{cons}; (2) Why is the strict positivity of data type definition
required in Coq?
...
\cinl{applyFun (con applyFun)} again, which indicates that this expression
will not terminate.
