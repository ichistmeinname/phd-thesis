...
Analogous to the definition of \cinl{append} at the end of \autoref{subsec:partial}, we need to split function into two parts: one part works on pure \cinl{List}s and takes care of patternmatching while the second part handles \cinl{Free}-lifted arguments only.
...
The \cinl{induction} tactic makes use the associated induction principle for the corresponding type.
An induction principle for a data type is an ordinary function of type \cinl{Prop} in Coq.
...
One of these arguments is the proposition \cinl{P}, which Coq proves automatically because it can be inferred from the goal we apply the function to.
...
Given a Peano number \cinl{n} and the induction hypothesis that the proposition already holds for that \cinl{n}, we need to prove that the proposition holds for \cinl{S n} as well.
