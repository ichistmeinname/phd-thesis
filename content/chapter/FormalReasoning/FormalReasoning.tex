...
types.
With \cinl{Free} we are well-suited to define a lifted version of
\cinl{List} as we wished to do at the beginning of this section.
...
The case distinction we uusing in the definition of \cinl{appendP'} is
...
to.
In the latter case we additionallymatch on the constructor \cinl{ext}
and have variable bindings \cinl{s} and \cinl{pf} for the shape and
...
\begin{minted}{coq}
  ...
  fys, fzs : Free Zero__S Zero__P (List Zero__S Zero__P A)
  ============================
  impure (ext s (fun p : Zero__P s => append (pf p) (append fys fzs))) =
  impure (ext s (fun p : Zero__P s => append (append (pf p) fys) fzs))
\end{minted}
...
of \cinl{impure (ext s pf)} on both sides of the equation, such that
...
\begin{minted}{coq}
  ...
  ============================
  impure (ext s (fun p : Ps s => append (pf p) (append fys fzs))) =
  impure (ext s (fun p : Ps s => append (append (pf p) fys) fzs))
\end{minted}
